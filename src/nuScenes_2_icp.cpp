...
int main (int argc, char** argv)
{
  ...
  myfile.open("/home/jimmy/catkin_ws/src/localization_competition/test/nuScenes_result2.csv", std::ios::out | std::ios::trunc);
  ...
  myfile.close();
  std::cerr << "Jump" << jump_count << "times." << std::endl;
...
void lidar_scan_cb (const sensor_msgs::PointCloud2::ConstPtr& pc2_msg)
{
  ...
  if (count <= 220)
    pass_z.setFilterLimits (1.0, 50.0);
  else{
    pass_z.setFilterLimits (3.0, 50.0); // leave the points on the floor
    //pass_z.setNegative (true);
  }
  pass_z.filter (*cloud_F_1);

  // Create the filtering object
  pcl::StatisticalOutlierRemoval<pcl::PointXYZ> sor;
  sor.setInputCloud (cloud_F_1);
  sor.setMeanK (50);
  ...
  */
  ...
  if (first_time){
    initial_guess(0,3) = 1716.4;
    initial_guess(1,3) = 1014.5;
    initial_guess(2,3) = -0.365639;
    // -2.18166156 rad/-125 degree
    initial_guess(0,0) = -0.5735764322610922;
    initial_guess(0,1) = 0.8191520471528083;
    initial_guess(1,1) = -0.5735764322610922;
    initial_guess(1,0) = -0.8191520471528083;
  ...
  /* This will return wrong angle. Still do not know the reason.
  ...
  */
  ...
  if (pos_error > 4 && abs(count - count_gps) < 3)
    score_is_bad = true;
